...
class VCI_CAN_STATUS(Structure):
    _fields_ = [
        ("ErrInterrupt",
         c_byte),  # interrupt record,will be cleared while reading
        ...
        ("regEWLimit",
         c_byte),  # CAN controller error alarm limitation register
...
class VCI_ERR_INFO(Structure):
    _fields_ = [
        ...
        ("Passive_ErrData", c_ubyte *
         3),  # error identification data when error has passive error
...
class VCI_INIT_CONFIG_EX(Structure):
    _fields_ = [
        ...
        ("CAN_ABOM",
         c_byte),  # auto off line management. 0: prohibit,1: enable
        (
            "CAN_NART", c_byte
        ...
        ("CAN_TXFP",
         c_byte),  # send priority management, 0: by ID, 1: by order
        (
            "CAN_RELAY", c_byte
...
def VCI_OpenDevice(DevType, DevIndex, Reserved):
    try:
        ...
        exit()
    return GinkgoLib.VCI_OpenDevice(c_uint(DevType), c_uint(DevIndex),
                                    c_uint(Reserved))
...
def VCI_InitCAN(DevType, DevIndex, CANIndex, pInitConfig):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_InitCAN(c_uint(DevType), c_uint(DevIndex),
                                 c_uint(CANIndex), pInitConfig)
...
def VCI_InitCANEx(DevType, DevIndex, CANIndex, pInitConfig):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_InitCANEx(c_uint(DevType), c_uint(DevIndex),
                                   c_uint(CANIndex), pInitConfig)
...
def VCI_ReadCANStatus(DevType, DevIndex, CANIndex, pCANStatus):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_ReadCANStatus(c_uint(DevType), c_uint(DevIndex),
                                       c_uint(CANIndex), pCANStatus)
...
def VCI_SetFilter(DevType, DevIndex, CANIndex, pFilter):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_SetFilter(c_uint(DevType), c_uint(DevIndex),
                                   c_uint(CANIndex), pFilter)
...
def VCI_GetReceiveNum(DevType, DevIndex, CANIndex):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_GetReceiveNum(c_uint(DevType), c_uint(DevIndex),
                                       c_uint(CANIndex))
...
def VCI_ClearBuffer(DevType, DevIndex, CANIndex):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_ClearBuffer(c_uint(DevType), c_uint(DevIndex),
                                     c_uint(CANIndex))
...
def VCI_RegisterReceiveCallback(DevIndex, pReceiveCallBack):
    'Register receive callback function'
    return GinkgoLib.VCI_RegisterReceiveCallback(c_uint(DevIndex),
                                                 pReceiveCallBack)
...
def VCI_StartCAN(DevType, DevIndex, CANIndex):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_StartCAN(c_uint(DevType), c_uint(DevIndex),
                                  c_uint(CANIndex))
...
def VCI_ResetCAN(DevType, DevIndex, CANIndex):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_ResetCAN(c_uint(DevType), c_uint(DevIndex),
                                  c_uint(CANIndex))
...
def VCI_Transmit(DevType, DevIndex, CANIndex, pSend, Len):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_Transmit(c_uint(DevType), c_uint(DevIndex),
                                  c_uint(CANIndex), pSend, c_uint(Len))
...
def VCI_Receive(DevType, DevIndex, CANIndex, pReceive, Len, WaitTime):
    ...
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args))
        exit()
    return GinkgoLib.VCI_Receive(c_uint(DevType), c_uint(DevIndex),
                                 c_uint(CANIndex), pReceive, c_uint(Len),
                                 c_uint(WaitTime))
